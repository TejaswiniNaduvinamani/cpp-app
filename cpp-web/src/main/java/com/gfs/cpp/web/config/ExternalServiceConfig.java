...
public class ExternalServiceConfig {
    ...
    @Value("${distributionCenter.service}")
    ...
    private static final String ITEM_CONFIGURATION_SERVICE_URL = "remoting/httpItemConfigurationService";
    ...
    private static final String DISTRIBUTION_SERVICE_QUERY_SERVICE_URL = "/organizationService/services/DistributionCenterQuery";
    ...
    @Bean
    public ItemConfigurationService itemConfigurationService() {
        final HttpInvokerProxyFactoryBean proxyFactoryBean = new HttpInvokerProxyFactoryBean();
        proxyFactoryBean.setServiceInterface(ItemConfigurationService.class);
        proxyFactoryBean.setServiceUrl(environment.getProperty(CPPConstants.ITEM_QUERY_HOST_URL) + ITEM_CONFIGURATION_SERVICE_URL);
        proxyFactoryBean.setHttpInvokerRequestExecutor(basicAuthenticationHttpInvokerRequestExecutor);
        proxyFactoryBean.afterPropertiesSet();
        return (ItemConfigurationService) proxyFactoryBean.getObject();
    }
    ...
    @Bean
    public DistributionSOAPClientService distributionSOAPService(){
        final JaxWsProxyFactoryBean jaxWsProxyFactoryBean = new JaxWsProxyFactoryBean();
        jaxWsProxyFactoryBean.setServiceClass(DistributionSOAPClientService.class);
        jaxWsProxyFactoryBean.setAddress(distributionServiceUrl + DISTRIBUTION_SERVICE_QUERY_SERVICE_URL);
        jaxWsProxyFactoryBean.setUsername(environment.getProperty("cpp.external.service.username"));
        jaxWsProxyFactoryBean.setPassword(environment.getProperty("cpp.external.service.password"));
        return (DistributionSOAPClientService) jaxWsProxyFactoryBean.create();
    }
